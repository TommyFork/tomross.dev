...
Currently configured project: `tomross-dev` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/tomross-dev/SETUP.md` to fill in
   `data/tomross-dev/INFO.md`.
...
`--project-id tomross-dev` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/tomross-dev/`.
