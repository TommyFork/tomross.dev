# Agent setup for `tomross-dev`

This is a deepsec scanning workspace. Project `tomross-dev` was just registered
(target: `..`). Setup is incomplete — `data/tomross-dev/INFO.md`
...
2. **Fill in `data/tomross-dev/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id tomross-dev
pnpm deepsec process --project-id tomross-dev
```
